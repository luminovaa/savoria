  ...
  export default function OrderDetailsScreen() {
    ...
    const printReceipt = async () => {
      ...
      let receiptText = `
    <C><B>TOKO ANDA</B></C>
    <C>Jl. Contoh No. 123</C>
    <C>Telp: 0812-3456-7890</C>
    <C>================================</C>
    
    <L><B>INVOICE:</B> ${order.invoice_number}</L>
    <L><B>TANGGAL:</B> ${formatDate(order.created_at)}</L>
    <L><B>KASIR:</B> ${order.user?.first_name || "-"}</L>
    <C>--------------------------------</C>
    `;
      ...
      orderItems.forEach((item) => {
        ...
        receiptText += `
    <L>${itemName}</L>
    <L>${item.quantity}x ${price} <R>${subtotal}</R></L>
    `;
      });
      ...
      receiptText += `
    <C>--------------------------------</C>
    <L><B>TOTAL:</B> <R><B>${formatCurrency(order.total_amount)}</B></R></L>
    <C>================================</C>
    <C>Terima kasih atas kunjungan Anda</C>
    <C>Barang yang sudah dibeli</C>
    <C>tidak dapat ditukar/dikembalikan</C>
    <C></C>
    <C></C>
    `;
                ...
                <FlatList
                  ...
                  renderItem={({ item }) => (
                    <TouchableOpacity
                      style={[
                        styles.deviceItem,
                        selectedDevice === item.inner_mac_address &&
