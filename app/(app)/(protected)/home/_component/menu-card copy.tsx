...
export default function MainCardMenu({
  ...
  try {
    ...
    const finalMenuData = staticMenuData.map((item: { id: string | number; }) => ({
      ...item,
    ...
    const filteredItems = finalMenuData.filter((item: { promo: any; promo_start: string | number | Date; promo_end: string | number | Date; }) => {
...
const updateQuantity = (id: string, delta: number) => {
  ...
  if (delta > 0 && (cart[id] || 0) >= item.stock!) {
    Alert.alert("Stok Tidak Cukup", "Jumlah di keranjang melebihi stok yang tersedia.");
    return;
  }
