...
func (s *Server) checkout(w http.ResponseWriter, r *http.Request) {
	...
	err = tx.QueryRow(r.Context(), `select id,invoice_number,customer,total_amount,changes,status from orders where client_order_id=$1`, input.ClientOrderID).
		Scan(&existing.OrderID, &existing.InvoiceNumber, &existing.Customer, &existing.TotalAmount, &existing.Changes, &existing.Status)
	if err == nil {
	...
	var orderID string
	err = tx.QueryRow(r.Context(), `insert into orders(client_order_id,invoice_number,customer,total,total_amount,paid,changes,user_id,payment_type,status)
		values($1,$2,$3,$4,$5,$6,$7,$8,$9,'completed') returning id`,
		input.ClientOrderID, invoice, input.Customer, totalItems, totalAmount, input.Paid, changes, currentUser(r).UserID, input.PaymentType).Scan(&orderID)
	if err != nil {
		conflictOrServer(w, err)
		return
	}
	...
	writeJSON(w, http.StatusCreated, checkoutResult{OrderID: orderID, InvoiceNumber: invoice, Customer: input.Customer, TotalAmount: totalAmount, Changes: changes, Status: "completed"})
}
...
func (s *Server) listOrders(w http.ResponseWriter, r *http.Request) {
	...
	rows, err := s.DB.Query(r.Context(), fmt.Sprintf(`select o.id,o.created_at,o.invoice_number,o.customer,o.total,o.total_amount,o.payment_type,o.status,o.user_id,u.full_name
	...
	for rows.Next() {
		var id, invoice, customer, payment, status, userID, fullName string
		var created time.Time
		var total int
		var amount float64
		_ = rows.Scan(&id, &created, &invoice, &customer, &total, &amount, &payment, &status, &userID, &fullName)
		result = append(result, map[string]any{"id": id, "created_at": created, "invoice_number": invoice,
			"customer": customer, "total": total, "total_amount": amount, "payment_type": payment, "status": status, "user_id": userID,
...
func (s *Server) getOrder(w http.ResponseWriter, r *http.Request) {
	...
	var orderID, invoice, customer, payment, status, userID, fullName string
	var created time.Time
	var total int
	var amount, paid, changes float64
	err := s.DB.QueryRow(r.Context(), `select o.id,o.created_at,o.invoice_number,o.customer,o.total,o.total_amount,coalesce(o.paid,0),o.changes,o.payment_type,o.status,o.user_id,u.full_name
		from orders o join users u on u.id=o.user_id where o.id=$1`, id).
		Scan(&orderID, &created, &invoice, &customer, &total, &amount, &paid, &changes, &payment, &status, &userID, &fullName)
	if err != nil {
		writeError(w, http.StatusNotFound, "order tidak ditemukan")
		return
	}
	itemsRows, err := s.DB.Query(r.Context(), `select oi.id,oi.order_id,oi.menu_id,oi.quantity,oi.price,oi.subtotal,m.name_menu,m.image_url from order_items oi join menus m on m.id=oi.menu_id where oi.order_id=$1`, id)
	if err != nil {
		conflictOrServer(w, err)
		return
	}
	...
	for itemsRows.Next() {
		var itemID, orderID, name, image string
		var menuID int64
		var quantity int
		var price, subtotal float64
		_ = itemsRows.Scan(&itemID, &orderID, &menuID, &quantity, &price, &subtotal, &name, &image)
		items = append(items, map[string]any{"id": itemID, "order_id": orderID, "menu_id": menuID, "quantity": quantity, "price": price, "subtotal": subtotal, "menu": map[string]any{"id": menuID, "name_menu": name, "images": image}})
	}
	order = map[string]any{"id": orderID, "created_at": created, "invoice_number": invoice, "total": total,
		"customer": customer, "total_amount": amount, "paid": paid, "changes": changes, "payment_type": payment, "status": status,
